...
class myLocalCluster(distributed.LocalCluster):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes a worker cluster with specified configurations and parameters.

        Args:
            ncpus: Number of CPUs to allocate for the cluster.
            min_workers: Minimum number of worker processes to maintain.
            max_workers: Maximum number of worker processes to allow.
            config: A dictionary containing configuration settings for the cluster.
            config_name: Name of the configuration file to use.
            persist_config: A boolean indicating whether to persist the configuration.
            **kwargs: Additional keyword arguments for worker configuration.

        Returns:
            None: This method does not return a value. It initializes the cluster.
        """
        # config
    ...
    def __enter__(self):
        ...
        This method is typically used in conjunction with a `with` statement to ensure proper acquisition and release of resources.

        Returns:
            The current instance of the object, allowing for continued interaction within a context manager.
        """
        return self
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        Handle the cleanup actions when exiting a context manager.

        This method is responsible for removing the configuration file if the
        object is not configured to persist it. It also closes the client connection
        and calls the exit method of the superclass.

        Args:
            exc_type: The exception type raised in the context block.
            exc_value: The exception instance raised in the context block.
            traceback: The traceback object associated with the exception raised.
...
class janeliaLSFCluster(dask_jobqueue.LSFCluster):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes a distributed computing client with specified configuration settings.

        This method sets up the environment for a distributed cluster of workers, including
        configuring paths and environment variables to optimize performance.

        Args:
            ncpus: The number of CPUs to allocate for the distributed computation.
            config: A dictionary containing configuration settings. Defaults to an empty dictionary.
            config_name: The name of the configuration file to be used. Defaults to DEFAULT_CONFIG_FILENAME.
            persist_config: A boolean indicating whether to persist the configuration settings. Defaults to False.
    ...
    def __enter__(self):
        ...
        This method is typically used in conjunction with the 'with' statement, allowing
        the resource managed by the object to be acquired and ensuring that it is available
        during the block enclosed by the 'with' statement.

        Returns:
            The current instance of the object, allowing for method chaining or attribute access.
        """
        return self

    def __exit__(self, exc_type, exc_value, traceback):
        """
        Exits the context by performing cleanup actions.

        This method is called when leaving a context managed by a class. It removes the configuration file
        if the `persist_config` attribute is set to False, closes the client, and delegates to the
        superclass's `__exit__` method.

        Args:
            exc_type: The exception type raised, if any.
            exc_value: The exception value raised, if any.
            traceback: The traceback object for the exception, if any.

        Returns:
            The result of the superclass's __exit__ method.
        """
        if not self.persist_config:
            _remove_config_file(self.config_name)
        self.client.close()
    ...
    def adapt_cluster(self, min_workers, max_workers):
        """
        Adapts the cluster configuration by setting the minimum and maximum number of worker jobs.

        Args:
            min_workers: The minimum number of workers to ensure for the cluster.
            max_workers: The maximum number of workers to allow for the cluster.
