...
class myLocalCluster(distributed.LocalCluster):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes a distributed client for the Cellpose application with specific configuration settings.

        Args:
            ncpus: The number of CPUs to use for distributed processing.
            config: A dictionary containing configuration settings for the distributed client. Defaults to an empty dictionary.
            config_name: The name of the configuration file. Defaults to DEFAULT_CONFIG_FILENAME.
    ...
    def __enter__(self):
        """
        Enables the use of the context manager.

        Returns the current instance of the object that uses this method.
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        """
        Exits the context manager, performing cleanup actions.

        This method handles cleanup when exiting a context, which may include
        removing a configuration file if it is not to be persisted and closing
...
class janeliaLSFCluster(dask_jobqueue.LSFCluster):
    ...
    def __enter__(self):
        """
        Enables the use of the object in a context manager.

        This method is called when entering a context manager. It allows
        the instance of the object to be returned, and typically,
        it prepares the object for use in a with statement.

        Returns:
            The instance of the object being used in the context.
        """
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        """
        Exits the context manager, cleaning up resources and removing the configuration file if necessary.

        Args:
            exc_type: The exception type raised in the context block, if any.
            exc_value: The exception instance raised in the context block, if any.
            traceback: The traceback object associated with the exception raised in the context block, if any.

        Returns:
            The return value from the superclass's __exit__ method, indicating whether to suppress the exception.
        """
    ...
    def adapt_cluster(self, min_workers, max_workers):
        """
        Adapts the cluster by setting the minimum and maximum number of workers.

        This method configures the cluster's worker settings based on the provided minimum and maximum worker counts.

        Args:
            min_workers: The minimum number of workers to utilize in the cluster.
            max_workers: The maximum number of workers to utilize in the cluster.
