...
def get_sigma(Tdown):
    """
    Computes the normalized correlation matrices for the provided tensors, highlighting the interactions between channels following the preprocessing steps.
...
def imstats(X, net1):
    """
    Calculates correlation matrices from the provided image tensor, enabling the exploration of interdependencies among various features in the data.
...
def loss_fn_per(img, net1, yl):
    """
    Calculates the perceptual loss by evaluating the discrepancies in feature statistics between a given input image and a reference clean image.
...
def test_loss(net0, X, net1=None, img=None, lbl=None, lam=[1.0, 1.5, 0.0]):
    """
    Computes the total loss for the model based on the input image, incorporating contributions from reconstruction, perceptual, and segmentation losses.
...
def train_loss(net0, X, net1=None, img=None, lbl=None, lam=[1.0, 1.5, 0.0]):
    """
    Calculates the overall loss during training by evaluating the output of an image processing model. This includes contributions from reconstruction loss, segmentation accuracy, and perceptual differences, allowing for a comprehensive assessment of model performance.
...
def img_norm(imgi):
    """
    Normalizes the pixel intensity values of the input image by scaling them according to specific percentile thresholds, improving overall contrast to facilitate enhanced visualization and subsequent analysis.
...
def add_noise(
    ...
    """
    Enhances the input image tensor by applying diverse transformations and incorporating noise, thereby increasing its variability and ensuring stronger performance in further processing tasks.
...
def random_rotate_and_resize_noise(
    ...
    """
    Applies a series of random transformations to the input data, including rotation, resizing, and noise addition. This method generates augmented images along with their respective labels, ensuring the preservation of essential spatial characteristics throughout the transformations.
...
def one_chan_cellpose(device, model_type="cyto2", pretrained_model=None):
    """
    Sets up a neural network tailored for processing single-channel inputs effectively.
...
class CellposeDenoiseModel:
    ...
    def __init__(
        ...
        """
        Initializes an instance of the class by creating DenoiseModel and CellposeModel objects.

        Args:
            gpu: A boolean indicating whether to use GPU for computation.
            pretrained_model: A boolean that specifies if a pretrained model should be used.
            model_type: A string that indicates the type of model to be used for Cellpose.
            restore_type: A string specifying the type of restoration to be applied by DenoiseModel.
            nchan: An integer representing the number of channels to be used in Cellpose.
            chan2_restore: A boolean indicating whether to enable restoration of a second channel.
            device: A string or None indicating the device to be used for computation.
...
class DenoiseModel:
    ...
    def __init__(
        ...
        """
        Initializes the denoising model with specified parameters, managing device configuration and the loading of models as needed. This setup ensures optimal performance based on the selected computational resources and model characteristics.

        This method sets up the neural network for processing, including the assignment of device settings
        and the loading of pretrained models if specified. Various parameters control the initialization
        behavior, ensuring compatibility with GPU usage and model types.

        Args:
            gpu: Indicates whether to use a GPU for computations.
            pretrained_model: The path or name of a pretrained model. Must be a string or a Path object.
            nchan: The number of input channels.
            model_type: Type of the model to be used.
            chan2: Indicates whether to load a second channel model.
            diam_mean: Mean diameter for the objects being processed.
            device: The device to be used for computations. If None, the device will be
                    assigned automatically based on available resources.
    ...
    def eval(
        ...
        """
        Improve the visual clarity and detail of the provided array or list of images using advanced image restoration techniques.
    ...
    def _eval(
        ...
        """
        Enhance the quality and clarity of input images through advanced restoration techniques, effectively handling a range of image dimensions and conditions.
...
def seg_train_noisy(
    ...
    """
    Performs training of the model with the specified training dataset and labels, allowing for the integration of noise augmentation techniques and dynamic learning rate adjustments. The function facilitates periodic saving of the model's state and can assess the model's performance on optional test datasets. It is important to ensure that the input data is appropriately normalized before invoking this method.
