...
class CellposeDenoiseModel:
    ...
    def __init__(
        ...
        """
        Initializes the model components for denoising and cell segmentation.

            This constructor sets up the necessary models for image processing tasks,
            including denoising and segmentation, based on specified configurations.

            Args:
                gpu: Indicates whether to use GPU acceleration.
                pretrained_model: Flag to specify if a pretrained cellpose model should be used.
                model_type: Type of the cellpose model to be used.
                restore_type: Type of the denoising model; defaults to "denoise_cyto3".
                nchan: Number of image channels to process; defaults to 2.
                chan2_restore: Indicates whether to also restore a second channel.
                device: Specifies the device for model computation (CPU or GPU).
...
class DenoiseModel:
    ...
    def __init__(
        ...
        Initializes the model with specified parameters.
            ...
            This method sets up the neural network for processing images,
            optionally loading a pretrained model if specified. It also
            configures the device (GPU or CPU) for computation and
            initializes various parameters related to the model architecture.

            Args:
                gpu: Flag indicating whether to use GPU for computation.
                pretrained_model: The path or name of a pretrained model to load.
                nchan: The number of input channels for the model.
                model_type: The type of model to be used if no pretrained model is provided.
                chan2: Flag indicating whether to load a second channel model.
                diam_mean: Mean diameter used for the model, influences how the model interprets objects.
                device: A specific device to run the model on, if not provided, will be assigned automatically.

            Returns:
                None: The method does not return any value but initializes the model attributes.
        """
        self.nchan = nchan
...
def train(
    ...
    """
    Train a neural network for image denoising and segmentation.

        This method trains a specified neural network using provided training and testing datasets,
        optionally saving the model at specified intervals. The training process includes various
        parameters for noise handling and learning rate adjustment, allowing for flexible model
        training configurations.

        Args:
            net: The neural network model to be trained.
            train_data: The training images, or None if using training files.
            train_labels: The labels corresponding to the training images.
            train_files: List of file paths to the training images if using files instead of direct data.
            test_data: The testing images, or None if using testing files.
            test_labels: The labels corresponding to the testing images.
            test_files: List of file paths to the testing images if using files instead of direct data.
            train_probs: Probabilities for sampling training images.
            test_probs: Probabilities for sampling testing images.
            lam: Weighting coefficients for loss functions.
            scale_range: Factor for scaling images.
            seg_model_type: Type of segmentation model to use.
            save_path: Directory for saving the model.
            save_every: Frequency of saving the model during training.
            save_each: Whether to save the model after each epoch.
            poisson: Parameters for Poisson noise simulation.
            beta: Parameters for additional noise modeling.
            blur: Parameters for blurring effects on images.
            gblur: Parameters for Gaussian blurring.
            iso: Boolean indicating if isotropic conditions apply.
            uniform_blur: Boolean indicating if uniform blurring should be applied.
            downsample: Factor for downsampling images.
            ds_max: Maximum downsampling factor.
            learning_rate: Initial learning rate for optimization.
            n_epochs: Number of epochs for training.
            weight_decay: Weight decay factor for regularization.
            batch_size: Number of samples in each training batch.
            nimg_per_epoch: Number of images to process per epoch.
            nimg_test_per_epoch: Number of images to process per testing epoch.
            model_name: Custom name for the saved model file.

        Returns:
            A tuple consisting of the filename where the model is saved, a list of training losses,
            and a list of testing losses throughout the training process.
    """
