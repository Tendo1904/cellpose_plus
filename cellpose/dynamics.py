...
def _extend_centers(T, y, x, ymed, xmed, Lx, niter):
    """
    Initiate a diffusion algorithm that progressively spreads values from a central location outward, updating the specified pixel values based on the average contributions from their neighboring pixels in multiple iterations.
...
def _extend_centers_gpu(
    ...
    """
    Computes flow fields by leveraging neighboring pixel data and specified mask centers, utilizing GPU acceleration for efficient diffusion processing.
...
def get_centers(masks, slices):
    """
    Determine the central coordinates of the specified labeled masks and compute their corresponding extents.
...
def masks_to_flows_gpu(masks, device=torch.device("cpu"), niter=None):
    """
    Convert labeled masks into flow representations by initiating a diffusion process from the median pixel within each mask, effectively capturing the underlying flow characteristics at each pixel location.
...
def masks_to_flows_gpu_3d(masks, device=None, niter=None):
    """
    Convert labeled masks into flow representations by utilizing a diffusion process that spreads from the central pixel within each mask.
...
def masks_to_flows_cpu(masks, niter=None, device=None):
    """
    Convert labeled masks into flow representations by applying diffusion processes that begin from the central pixel of each mask. The flow is derived from the gradients calculated from the resulting diffusion density map.
...
def masks_to_flows(masks, device=torch.device("cpu"), niter=None):
    """
    Convert labeled masks into flow representations by initiating a diffusion process from a defined central pixel, identified as the nearest point to the average coordinates of the mask pixels. The diffusion outcome is subsequently transformed into flow vectors by computing the gradients of the generated diffusion density map.
...
def labels_to_flows(labels, files=None, device=None, redo_flows=False, niter=None):
    """
    Converts a collection of label masks or precomputed flow data into a detailed flow representation, facilitating the training process of machine learning models.
...
def map_coordinates(I, yc, xc, Y):
    """
    Performs bilinear interpolation on the input image "I" to transform the specified coordinates "yc" and "xc" into the output array "Y".
...
def steps_interp(dP, inds, niter, device=torch.device("cpu")):
    """
    Calculates refined pixel positions in 2D or 3D space through iterative dynamics, employing interpolation techniques to smooth transitions and improve positioning accuracy.
    ...
    if (device.type == "cuda" or device.type == "mps") or ndim == 3:
        ...
        for n in range(ndim):
            ...
            im[0, ndim - n - 1] = torch.from_numpy(dP[n]).to(
                device, dtype=torch.float32
            )
...
def steps3D(p, dP, niter):
    """
    Performs iterative updates to the positions of pixels in three-dimensional space by applying velocity field adjustments over a defined number of iterations.
...
def steps2D(p, dP, niter):
    """
    Update the positions of pixels iteratively, adjusting their locations according to the specified flow dynamics over a defined number of iterations.
...
def follow_flows(dP, inds, niter=200, interp=True, device=torch.device("cpu")):
    """
    Update the positions of pixels based on flow vectors through iterative dynamics, accommodating both 2D and 3D spatial configurations.
...
def remove_bad_flow_masks(masks, flows, threshold=0.4, device=torch.device("cpu")):
    """
    Assess and filter out masks that do not meet flow consistency standards based on computed flow errors, thereby retaining only those masks deemed trustworthy for subsequent analysis.
...
def max_pool3d(h, kernel_size=5):
    """
    Performs a memory-efficient 3D max pooling operation on the input tensor. The method applies a specified kernel size to compute the maximum values within the defined neighborhood, efficiently handling edge cases via padding. The kernel size must be an odd number greater than or equal to 3.
...
def max_pool1d(h, kernel_size=5, axis=1, out=None):
    """
    Applies 1D max pooling to the input tensor, ensuring efficient memory usage. This method uses a stride of 1 and implements padding that is half the size of the kernel, which is required to be an odd integer with a minimum value of 3.
...
def get_masks_torch(pt, inds, shape0, rpad=20, max_size_fraction=0.4):
    """
    Create masks by analyzing the spatial distribution of pixels following dynamic adjustments. This method constructs histograms of pixel positions, identifies primary peaks, and expands the masks to encompass all pixels associated with those peaks, while filtering out any masks exhibiting significant flow discrepancies.
...
def resize_and_compute_masks(
    ...
    """
    Create masks from the dynamics flow field and cell probability data, offering the option to adjust the size of the masks based on specified dimensions.
...
def compute_masks(
    ...
    """
    Create binary masks from the dynamics flow field and cell probability data through iterative analysis, applying optional constraints on size to refine the output.
...
def get_masks_orig(p, iscell=None, rpad=20, max_size_fraction=0.4):
    """
    Create masks from the final pixel positions derived from dynamic processes, employing a histogram-based technique for identification and extension of mask regions.
