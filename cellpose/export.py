...
def load_bioimageio_cpnet_model(path_model_weight, nchan=2):
    """
    Load a CPNet model from BioImageIO with specified parameters.

        This method initializes a CPNet model using the provided model weights
        and channel configuration. It sets the model to evaluation mode,
        preparing it for inference.

        Args:
            path_model_weight: The file path to the model weights for loading the CPNet.
            nchan: The number of input channels for the CPNet architecture.

        Returns:
            A tuple containing:
                - The initialized CPNet model.
                - A dictionary of keyword arguments used for the CPNet initialization.
    """
    cpnet_kwargs = {
...
def descr_gen_input(path_test_input, nchan=2):
    """
    Generate a description of the input tensor for testing.

        This method constructs an input tensor description with specified
        dimensions and data types, based on the provided parameters. It is
        useful for setting up input data structures required during testing
        of machine learning models.

        Args:
            path_test_input: The file path to the test input data.
            nchan: The number of channels in the input data. Defaults to 2.

        Returns:
            An InputTensorDescr object that includes the configuration of
            the input tensor, including its dimensions and data source.
    """
    input_axes = [
...
def descr_gen_output_flow(path_test_output):
    """
    Generates the output tensor description for a given test output path.

        This method creates an output tensor description using predefined axes configurations
        and associates it with a specified test output path.

        Args:
            path_test_output: The path to the test output file.

        Returns:
            An OutputTensorDescr object containing the ID of the tensor, its axes
            configuration, and a description of the test tensor file.
    """
...
def descr_gen_output_downsampled(path_dir_temp, nbase=None):
    """
    Generate descriptions of downsampled output tensors.

        This method creates descriptions for a set of downsampled output tensors based on the specified base sizes.
        If no base sizes are provided, default values are used. The method constructs the axes configuration for each
        tensor and maps them to file paths where the downsampled tensors will be stored.

        Args:
            path_dir_temp: The directory path where the downsampled tensor files will be saved.
            nbase: A list of integers specifying the base sizes for downsampling. If None, default values of
                   [32, 64, 128, 256] are used.

        Returns:
            A list of OutputTensorDescr objects, each containing tensor IDs, axes configuration, and file
            descriptions for the downsampled tensors.
    """
    if nbase is None:
        nbase = [32, 64, 128, 256]
...
def descr_gen_output_style(path_test_style, nchannel=256):
    """
    Generate a description of the output style tensor.

        This method creates and returns an output tensor description for a style
        tensor based on the provided file path and the number of channels.

        Args:
            path_test_style: The file path where the style tensor is located.
            nchannel: The number of channels for the output tensor. Defaults to 256.

        Returns:
            An OutputTensorDescr object that describes the style tensor output.
    """
...
def descr_gen_arch(cpnet_kwargs, path_cpnet_wrapper=None):
    """
    Generates a PyTorch architecture descriptor from a specified file.

        This method constructs a PyTorch architecture descriptor using the
        provided keyword arguments. If no path to the wrapper file is provided,
        it defaults to using the "resnet_torch.py" file located in the same
        directory as this script.

        Args:
            cpnet_kwargs: A dictionary containing keyword arguments for the
                architecture descriptor.
            path_cpnet_wrapper: An optional path to the file containing the
                architecture definition. If not provided, defaults to
                "resnet_torch.py".

        Returns:
            A PyTorch architecture descriptor which encapsulates the
            architecture defined in the specified file, initialized with the
...
def descr_gen_documentation(path_doc, markdown_text):
    ...
    Generates a documentation file in markdown format.
        ...
        This method writes the provided markdown text to a file specified by the
        given path. The file is created or overwritten if it already exists.

        Args:
            path_doc: The file path where the markdown documentation will be saved.
            markdown_text: The markdown content to be written to the file.
...
def parse_args():
    """
    Parse command line arguments for BioImage.IO model packaging for Cellpose.

        This method sets up an argument parser and defines the necessary command
        line arguments required to package a model for use with BioImage.IO,
        specifically for the Cellpose application.
...
def main():
    """
    Main entry point for the model preparation and testing workflow.

        This method orchestrates the process of parsing user arguments, setting up paths,
        downloading and normalizing images, loading a pre-trained model, testing the model,
        saving outputs, generating model descriptions, packaging the model for BioImage.IO,
        and displaying test summaries.
