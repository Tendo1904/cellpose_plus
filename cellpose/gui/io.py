...
def _init_model_list(parent):
    """
    Initializes the model list for the specified parent object.

    This method creates the model directory if it does not exist, sets the path for the model list
    in the parent object, and retrieves the user-defined models.
...
def _add_model(parent, filename=None, load_model=True):
    """
    Adds a model to the GUI by loading it from a specified file or prompting the user to select a file if none is provided.

    Args:
        parent: The parent widget that contains the ModelChooseC for selecting models.
        filename: The path to the model file. If not provided, a file dialog will be opened to select a file.
        load_model: A flag indicating whether to load the model after adding it to the GUI.
...
def _get_image_size(filename):
    """
    Retrieves the image size ratio in millimeters per pixel from the associated metadata file.

    This method checks for the existence of a metadata XML file in specific directories based on the provided filename. It extracts the image dimensions from the XML and calculates the size ratio.

    Args:
        filename: The path to the image file for which the size ratio is to be calculated.

    Returns:
        The size ratio in millimeters per pixel as a float.

    Raises:
        Exception: If the metadata file is not found or if there are issues parsing the XML.
    """
...
def _set_image_size(parent):
    """
    Sets the image size in millimeters for the given parent object based on the image
    filename.

    Args:
        parent: An object that contains a filename attribute and a pixTomicro
            attribute for displaying the converted size.
...
def _save_outlines(parent):
    """
    Saves 2D outlines to a text file if the specified conditions are met.

    This method extracts the filename from the parent object, checks if the NZ property is equal to 1,
    and if so, it retrieves outlines from the cellpix of the parent and saves them to a text file in a
    format compatible with ImageJ. If NZ is not equal to 1, an error message is printed indicating
    that 3D outlines cannot be saved.

    Args:
        parent: The parent object that contains the filename and cellpix data.
