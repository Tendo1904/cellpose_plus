...
def _init_model_list(parent):
    """
    Initialize the model list for the specified parent object.

        This method creates the necessary directory for storing models, assigns
        the path for the model list to the parent object, and retrieves the
        user-defined models.

        Args:
            parent: The object which will receive the model list path and
                    user model strings. This object should have attributes
                    `model_list_path` and `model_strings` to store the relevant
                    data.
...
def _add_model(parent, filename=None, load_model=True):
    """
    Add a model to the GUI from the specified file.

        This method opens a file dialog to select a model file if no filename
        is provided. It adds the model to the GUI component and updates the
        list of available models. If the model already exists, it is removed
        from the list before being added again.

        Args:
            parent: The parent object that contains GUI components.
            filename: The filename of the model to be added. If None,
                      a file dialog will be opened to select the model.
            load_model: A boolean indicating whether to load the model
                        after adding it. Defaults to True.
...
def _remove_model(parent, ind=None, verbose=True):
    """
    Removes a model from the parent component's model selection.

        This method removes a model specified by its index from the
        ModelChooseC dropdown of the parent. If no index is provided,
        it defaults to the current selected index. After removal, it
        updates the dropdown to reflect the change and deletes the
        corresponding model string.

        Args:
            parent: The parent component containing the model selection.
            ind: The index of the model to be removed. If not provided,
                 the current index will be used.
            verbose: A flag indicating whether to output verbose messages.
...
def _get_image_size(filename):
    """
    Retrieve the size ratio of an image based on its metadata.

        This method processes a given filename to extract the image's size ratio in millimeters per pixel
        by reading the corresponding properties stored in an XML file. If the expected XML file is not found
        in the specified location, it attempts to access an alternative path.

        Args:
            filename: The full path of the image file for which the size ratio is to be determined.

        Returns:
            A float representing the size ratio in millimeters per pixel
            if the metadata file is successfully read, otherwise, it returns None and prints an error message.
    """
...
def _set_image_size(parent):
    """
    Sets the image size for the given parent object.

        This method retrieves the pixel to millimeter size of the image
        associated with the parent object and updates the parent's attribute
        accordingly. It also updates the textual representation of the size
        in the user's interface.

        Args:
            parent: The parent object that contains the image and UI elements.
...
def _save_outlines(parent):
    """
    Saves 2D outlines to a text file if the parent object is compatible.

        This method extracts outlines from a parent object and saves them to a
        text file if the parent object's NZ attribute is equal to 1. If NZ is
        not equal to 1, an error message is printed.

        Args:
            parent: The parent object containing the filename and cellpix attributes.
                    The filename is used to determine the base name for the output file,
                    and cellpix contains the outline data to be processed.
