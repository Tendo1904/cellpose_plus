...
def main():
    """
    Main entry point for processing images with cellpose parameters.

    This method sets up command-line arguments for specifying input images,
    algorithm settings, and options for how to process the images. It then
    reads the specified images, possibly performs normalization and sharpening,
    and saves the processed images into a designated training folder.

    Parameters:
        dir: Folder containing data to run or train on.
        image_path: If given and --dir not provided, run on a single image instead of folder (cannot train with this option).
        look_one_level_down: Run processing on all subdirectories of the current folder.
        ...
        chan: Channel to segment; options are GRAY, RED, GREEN, or BLUE (default is GRAY).
        chan2: Nuclear channel (optional, if cyto); options are NONE, RED, GREEN, or BLUE (default is NONE).
        ...
        crop_size: Size of random crop to save (default is 512).
