...
def logger_setup(
    ...
    """
    Sets up a logger that writes log output to a specified file and optionally to standard output.

    Args:
        cp_path: The path to the directory where the log file will be created. Defaults to ".cellpose".
        logfile_name: The name of the log file. Defaults to "run.log".
        stdout_file_replacement: An optional parameter that, if provided, replaces standard output with a file handler.
...
def check_dir(path):
    """
    Checks if a directory exists at the specified path, and creates it if it does not.

    Args:
        path: The path of the directory to check and possibly create.
...
def outlines_to_text(base, outlines):
    ...
    Converts a list of outlines into a text format and saves it to a file.

    This method takes a base filename and a list of outline objects, flattens the outlines into coordinate format,
    and writes them to a text file named with the base name followed by "_cp_outlines.txt".

    Args:
        base: The base filename to use for the output file.
        outlines: A list of outline objects to be processed and written to the file.
...
def load_dax(filename):
    """
    Loads a DAX file and its associated metadata from an INF file.

    The method retrieves the image dimensions and number of frames from the INF file, and then loads the DAX file into memory as a NumPy array. If the INF file is not found, a critical log message is generated, and the method returns None. The method assumes default dimensions if they cannot be determined from the INF file.

    Args:
        filename: The path to the DAX file to be loaded.

    Returns:
        np.ndarray: A NumPy array containing the image data from the DAX file, or None if the INF file is not found.
    """
