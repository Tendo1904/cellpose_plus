...
def logger_setup(
    ...
    Sets up the logging configuration for the application.
        ...
        This method creates a directory for storing log files if it does not exist,
        and configures the logging handlers to write logs to a specified log file and
        optionally replace standard output with a log file.

        Args:
            cp_path: The path to the directory where the log file will be created.
            logfile_name: The name of the log file to write logs to.
            stdout_file_replacement: An optional parameter that specifies a file to
                replace standard output. If None, logs will also be printed to the console.

        Returns:
            A tuple containing the configured logger and the path of the log file.
    """
...
def check_dir(path):
    ...
    Check if a directory exists and create it if it does not.
        ...
        This method checks if the specified directory exists at the given path.
        If the directory does not exist, it creates a new directory at that path.

        Args:
            path: The path to the directory that needs to be checked.
...
def outlines_to_text(base, outlines):
    """
    Converts a list of outline objects into a text file.

        This method takes a base file name and a list of outline objects, flattens the coordinates of each outline,
        and writes them to a text file with a specified base name.

        Args:
            base: The base name for the output text file, which will have "_cp_outlines.txt" appended to it.
            outlines: A list of outline objects that contain coordinate data to be written to the file.

        Returns:
            None: This method does not return any value, but creates a text file with the outlines.
    """
...
def load_dax(filename):
    """
    Load a DAX file and return its image data as a NumPy array.

        This function retrieves image data from a DAX file, utilizing an associated `.inf` metadata file
        to extract information on the dimensions and number of frames. It warns the user if specific metadata
        cannot be determined, defaulting to a size of 256x256 if necessary.

        Args:
            filename: The path to the DAX file to be loaded.

        Returns:
            A NumPy array containing the image data from the DAX file, or None if the associated `.inf`
            file is not found.
    """
