...
def model_path(model_type, model_index=0):
    """
    Constructs the model path based on the provided model type and index.

        This method generates a model path string by combining the model type
        and an optional model index. If the model type is 'cyto', 'cyto2', or
        'nuclei', it creates a basename with the structure '{model_type}torch_{model_index}'.
        Otherwise, it uses the model type as the basename directly.

        Args:
            model_type: The type of the model which dictates the structure of the path.
            model_index: An optional index for the model, defaulting to 0.

        Returns:
            A string representing the complete path to the model in the cache.
    """
    torch_str = "torch"
...
def size_model_path(model_type):
    """
    Generate the file path for the size model based on the model type.

        This method constructs and returns the path to a size model file
        based on the specified model type. It supports specific model types
        and checks for file existence in the local filesystem.

        Args:
            model_type: A string indicating the model type. Supported
            types include "cyto", "nuclei", "cyto2", and "cyto3". If the
            provided model type is not one of these, the method checks for
            an existing file with the name pattern "<model_type>_size.npy".

        Returns:
            A string representing the file path to the size model.

        Raises:
            FileNotFoundError: If a size model file for the specified
...
def cache_model_path(basename):
    """
    Caches a model file by downloading it from a specified URL if it does not already exist locally.

    This method ensures that the model is available in the designated directory by downloading it from
    a remote source. If the file is already present, it simply returns the path to the cached file.

    Args:
        basename: The name of the model file to cache.
...
def get_user_models():
    """
    Retrieve a list of user models from a specified file.

        This method checks if a certain file containing user model names exists.
        If it does, the method reads the file, extracts the model names line by
        line, and returns them as a list.

        Returns:
            A list of user model names read from the specified file. If the file
...
class Cellpose:
    ...
    def __init__(
        ...
        """
        Initializes the Cellpose model with specified parameters.

            This method sets up the device (CPU or GPU), initializes the Cellpose model with
            the specified model type, channel count, and backbone, and prepares the size model.

            Args:
                gpu: Indicates whether to use GPU for computation.
                model_type: The type of the model to be used (e.g., "cyto", "nuclei", "cyto2", "cyto3").
                nchan: The number of input channels. Defaults to 2.
                device: The device to be used for computation. If None, the appropriate device will be assigned.
                backbone: The backbone architecture to be used for the model. Defaults to "default".
