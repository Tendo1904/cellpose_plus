...
def model_path(model_type, model_index=0):
    """
    Generates the model path based on the specified model type and index.

    This method constructs a base file name for the model depending on the given model type and index.
    If the model type is "cyto", "cyto2", or "nuclei", it generates a name that includes the model type
    and the model index. For other model types, it simply uses the provided model type as the base name.

    Args:
        model_type: The type of the model which determines the naming convention.
        model_index: An optional index used to differentiate between models of the same type.
...
def size_model_path(model_type):
    """
    Determines the file path for the size model based on the specified model type.

    This method constructs a filename for the size model based on the `model_type` provided.
    If the model type is one of the predefined options (cyto, nuclei, cyto2, or cyto3), it generates
    a filename based on the model type. If the model type doesn't match the predefined options,
    it checks if a size model file exists for the given path.

    Args:
        model_type: The type of model for which to determine the size model path.

    Returns:
        A string representing the full path of the size model file.

    Raises:
        FileNotFoundError: If the specified size model file does not exist.
    """
    torch_str = "torch"
    if (
        model_type == "cyto"
        ...
        if os.path.exists(model_type) and os.path.exists(model_type + "_size.npy"):
            ...
            raise FileNotFoundError(
                f"size model not found ({model_type + '_size.npy'})"
            )


def cache_model_path(basename):
    """
    Caches a model file by downloading it from a specified URL if it is not already present locally.

    Args:
        basename: The name of the model file to be cached.

    Returns:
        The file path to the cached model.
    """
...
def get_user_models():
    """
    Retrieves a list of user models from a specified file.

    This method checks for the existence of a file at the path defined by
    MODEL_LIST_PATH. If the file exists and contains lines, it reads the lines,
    removes any trailing whitespace, and returns them as a list of model strings.

    Returns:
        A list of strings containing the user models.
    """
    model_strings = []
...
class Cellpose:
    ...
    def __init__(
        ...
        """
        Initialize a Cellpose instance with specified configurations.

        This constructor assigns the device to be used (GPU or CPU), configures the model type,
        and sets various parameters such as the number of channels and the backbone of the model.
        It also initializes the Cellpose model and size model based on the provided settings.

        Args:
            gpu: A flag to indicate whether to use a GPU for computations.
            model_type: The type of model to use, defaults to "cyto3".
            nchan: The number of input channels for the model, defaults to 2.
            device: The device to run the model on; if not provided, it defaults to the assigned device.
            backbone: The backbone architecture to use for the model, defaults to "default".
...
def get_model_params(
    pretrained_model, model_type, pretrained_model_ortho, default_model="cyto3"
):
...
class SizeModel:
    ...
    def __init__(self, cp_model, device=None, pretrained_size=None, **kwargs):
        """
        Initialize a Cellpose model with specified parameters.

        This constructor sets up the device (GPU or CPU), model type, channel count, and backbone
        for the Cellpose model. It also initializes the size model based on the provided model type.

        Parameters:
        gpu: A boolean indicating whether to use GPU for computations.
        model_type: A string that specifies the type of model to use; defaults to "cyto3".
        nchan: An integer representing the number of input channels; defaults to 2.
        device: A string or None that specifies the device to use; if None, assigns based on available hardware.
        backbone: A string to specify the backbone network; defaults to "default".
