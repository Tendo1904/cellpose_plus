...
def pad_to_rgb(img):
    """
    Pads a grayscale or non-RGB image to ensure it has three color channels.

        This method checks the dimensions of the input image and modifies it as necessary to ensure
        that it has three channels. If the input is a grayscale image (2D), it will be duplicated
        across three channels. If it has fewer than three channels, it will randomly shuffle the
        available channels and optionally insert a blank channel to reach three total channels.

        Args:
            img: The input image to be padded, which may be a 2D grayscale image or a multi-channel image.

        Returns:
            The padded image that has three channels, conforming to RGB format.
    """
...
def convert_to_rgb(img):
    ...
    Convert an image to RGB format.
        ...
        This method takes an image and converts it to RGB format if it is not already in that format.
        If the input image is grayscale (2D), it creates a 3-channel version by duplicating the single channel.
        If the image has fewer than 3 channels, it averages the channels and normalizes it before duplicating.

        Args:
            img: The input image, which could be a 2D grayscale image or an image with fewer than 3 channels.

        Returns:
            The converted image in RGB format, with three channels.
    """
    if img.ndim == 2:
        img = img[np.newaxis, :, :]
