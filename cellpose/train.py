...
def _loss_fn_seg(lbl, y, device):
    """
    Calculates the total loss by evaluating the difference between true labels and predicted outputs, combining mean squared error for continuous predictions and binary cross-entropy for classification tasks.
...
def _get_batch(
    ...
    """
    Fetch a designated subset of images along with their associated labels, guided by the specified indices.
...
def _reshape_norm(
    ...
    """
    Transforms and scales the input data to enhance its compatibility and readiness for subsequent analyses.
...
def _process_train_test(
    ...
    """
    This method facilitates the organization and preparation of training and testing datasets by managing file paths, ensuring dimensional consistency, transforming labels into flows, calculating mask diameters, and applying normalization techniques. It guarantees that the datasets are primed for effective analysis and model training activities.
...
def train_seg(
    ...
    """
    Trains the neural network with specified image datasets, optimizing it for accurate prediction of segmentations within the input images.
...
def train_size(
    ...
    """
    Facilitates the training of a neural network model to estimate size parameters using specified training and testing datasets, while optionally leveraging pre-trained weights to enhance model accuracy.
