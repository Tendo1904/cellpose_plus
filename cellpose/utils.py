...
class TqdmToLogger(io.StringIO):
    ...
    def write(self, buf):
        """
        Writes the provided buffer to the object's internal storage after stripping whitespace characters.

        Args:
            buf: The input buffer to be stored, which may contain leading or trailing whitespace.
...
def hsv_to_rgb(arr):
    """
    Converts an array of HSV values to RGB values.

    This method takes an array of HSV color values, unrolls the channels,
    and applies the conversion from HSV to RGB. The result is a new array
    with the corresponding RGB values.

    Args:
        arr: A numpy array with the last dimension representing HSV channels.

    Returns:
        A numpy array containing the RGB values corresponding to the input HSV values.
    """
...
def download_font():
    """
    Download the DejaVuSans font file if it is not already present in the user's home directory.

    This method checks for the existence of the DejaVuSans.ttf font file at a specific path. If the file is not found, it downloads the font from a predefined URL.
