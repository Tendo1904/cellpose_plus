...
class TqdmToLogger(io.StringIO):
    ...
    def __init__(self, logger, level=None):
        """
        Initializes the TqdmToLogger instance with a logger and an optional logging level.

            This constructor sets up the logger for Tqdm and allows an optional
            logging level to be specified. If no level is provided, it defaults to
            INFO.

            Args:
                logger: The logger instance used to log messages.
                level: An optional logging level; defaults to INFO if not provided.
    ...
    def write(self, buf):
        """
        Writes the given buffer to the instance variable after stripping unwanted characters.

            This method takes a string buffer, removes any leading or trailing whitespace,
            carriage return, and tab characters, and then stores the cleaned string in an
            instance variable.

            Args:
                buf: The input string buffer that will be processed and stored.
    ...
    def flush(self):
        """
        Flush the buffered log messages to the logger.

            This method sends the contents of the buffer to the logger at
            the specified log level. It is typically called to ensure that
            any accumulated log messages are output immediately.
...
def rgb_to_hsv(arr):
    ...
    Convert an RGB image to HSV color space.
        ...
        This method takes an array representing an image in RGB format and converts it to the HSV color space.

        Args:
            arr: A numpy array representing an RGB image where the last dimension contains the red, green, and blue channels.

        Returns:
            A numpy array in HSV color space, with the same dimensions as the input array.
    """
...
def hsv_to_rgb(arr):
    """
    Convert an HSV image array to an RGB image array.

        This method takes an input array in the HSV color space and converts it to the RGB color space.
        The input array is expected to have the last dimension representing the HSV channels.

        Args:
            arr: A multi-dimensional array where the last dimension represents
                  the HSV channels (Hue, Saturation, Value).

        Returns:
            A multi-dimensional array where the last dimension represents
            the RGB channels (Red, Green, Blue).
    """
...
def download_font():
    """
    Download the DejaVuSans font file if it is not already present.

        This method checks if the DejaVuSans.ttf font file exists in the
        user's cellpose directory. If the file is not found, it downloads
        the font from a specified URL and saves it to the appropriate location.
