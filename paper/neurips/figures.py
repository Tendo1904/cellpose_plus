...
def fig1(imgs_norm, masks_true, dat, timings, save_fig=False):
    """
    Generate and display visualizations of image validation results.

        This method creates a series of subplots to visualize validation images,
        their corresponding masks, F1 scores, and runtime statistics for different
        segmentation models. It organizes the visuals in a grid layout and can save
        the generated figure as a PDF file if specified.

        Args:
            imgs_norm: The normalized images to be displayed.
            masks_true: The ground truth masks for the images.
            dat: A dictionary containing various data for multiple segmentation models,
                 including thresholds and F1 scores.
            timings: An array of runtimes for each model across several images.
            save_fig: A flag indicating whether to save the generated figure as a PDF file.

        Returns:
            None: This method does not return a value but produces a visual output.
    """
    ...
    ax.set_xticklabels([])
    ax.text(-0.1, 1.05, "Test set runtimes", fontsize="large", transform=ax.transAxes)
    ...
    ax.text(
        -0.1, 1.05, "Validation set scores", fontsize="large", transform=ax.transAxes
    )
    il = plot_label(ltr, il, ax, transl1, fs_title)
...
def fig2(imgs_norm, masks_true, dat, type_names, types, emb, emb_test, save_fig=False):
    """
    Generates a multi-panel figure to compare ground truth outlines with model predictions.

        This method creates a visual comparison of images with their corresponding
        ground truth and predicted outlines for different models, as well as a t-SNE
        representation of image styles based on embeddings. The figure is structured
        into multiple subplots for clarity.

        Args:
            imgs_norm: Normalized images to be displayed in the figure.
            masks_true: Ground truth masks for the images.
            dat: Dictionary containing model predictions and associated metrics.
            type_names: Names of the different image types or models.
            types: Types associated with each embedding for color coding.
            emb: Embeddings corresponding to the training set for t-SNE visualization.
            emb_test: Embeddings corresponding to the validation and test sets.
            save_fig: Boolean indicating whether to save the figure as a PDF.

        Returns:
            None: This method does not return any value. It instead generates and
            potentially saves a figure visualizing the provided data.
    """
    ids = [0, 3, 56, 55, 81, 75]
    ...
    for j in range(len(ids)):
        ...
        for o in outlines_gt[j]:
            ax.plot(
                o[:, 0], o[:, 1], color=[0.7, 0.4, 1], lw=2, ls="-", rasterized=True
            )
        for o in outlines_cp[j]:
            ax.plot(
                o[:, 0], o[:, 1], color=[1, 1, 0.3], lw=1.5, ls="--", rasterized=True
            )
        ax.set_ylim(ylims[j])
        ax.set_xlim(xlims[j])
        ax.axis("off")
        ...
        if j == 0:
            ax.text(
                -0.1, 0.5, "Mediar", rotation=90, va="center", transform=ax.transAxes
            )
    ...
    ax.scatter(
        emb[:-100, 1], emb[:-100, 0], color=0.8 * np.ones(3), s=1, rasterized=True
    )
    s1 = ax.scatter(
        emb[-100:, 1],
        emb[-100:, 0],
