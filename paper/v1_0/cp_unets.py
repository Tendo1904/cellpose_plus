...
def get_pretrained_models(
    ...
    """
    Retrieve paths of pretrained models based on specified configurations.

        This method constructs a model string based on the given parameters and returns a sorted list of file paths
        that match the generated model string pattern. The user can specify whether to use a UNet architecture,
        the number of classes, and various style configurations related to the model.

        Args:
            model_root: The root path where the model files are located.
            unet: A flag indicating whether to use a UNet architecture.
            nclass: The number of classes for the model.
            residual: A flag determining whether to include residual connections.
            style: A flag to choose the style of the model.
            concatenate: A flag to specify whether concatenation should be applied.

        Returns:
            A list of strings representing the paths of the pretrained models that were found.
    """
    sstr = ["off", "on"]
...
def test_timing(test_root, save_root):
    """
    Evaluates the performance timing of the Cellpose model on a test image dataset.

        This method loads test images and corresponding predicted diameters, rescales the test images,
        and measures the time taken to run the Cellpose model evaluation on the images using different devices.
        The timing results are stored for further analysis.

        Args:
            test_root: The directory path where the test images and predicted diameters are located.
            save_root: The directory path where results or files may be saved (currently not used in the method).

        Returns:
            A numpy array containing the timing results for the evaluation, structured by device and batch size.
    """
    itest = 14
