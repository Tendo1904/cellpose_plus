...
def get_pretrained_models(
    ...
    """
    Generates a list of paths to pretrained models based on the specified parameters.

    This method constructs a model string according to the provided arguments and retrieves
    the paths of pretrained model files that match the constructed string pattern.

    Args:
        model_root: The root directory where the model files are located.
        unet: A flag indicating whether to use a U-Net architecture (1 for on, 0 for off).
        nclass: The number of classes for the model.
        residual: A flag indicating whether to include residual connections (1 for on, 0 for off).
        style: A flag indicating the style of the model (1 for a specific style, 0 for default).
        concatenate: A flag indicating whether to use concatenation (1 for on, 0 for off).

    Returns:
        A list of paths to the pretrained model files that match the constructed model string.
    """
    sstr = ["off", "on"]
...
def test_timing(test_root, save_root):
    """
    Tests the timing of the Cellpose model evaluation on input images.

    This method reads a specified test image and its associated predicted diameters, resizes the image according to the predicted diameters, and evaluates the performance of the Cellpose model on the resized image using different devices and batch sizes. It measures the time taken for evaluations and prints the maximum mask value for each run.

    Args:
        test_root: The path to the directory containing the test images and predicted diameters.
        save_root: The path to the directory where any results should be saved (currently unused in the method).
