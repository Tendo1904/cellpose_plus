...
def suppfig_cellpose_params(test_root, save_root, save_figure=False):
    """
    Generates supplementary figures for Cellpose parameters and saves them.

        This method visualizes the average precision differences of Cellpose model outputs and
        performs statistical tests, displaying the results in a series of histograms and plots.
        It also allows for saving the generated figures to a specified directory.

        Args:
            test_root: The directory path containing test images and masks.
            save_root: The directory path where Cellpose output data is saved.
            save_figure: A boolean flag indicating whether to save the generated figure.
                         Defaults to False.

        Returns:
            None: This method does not return any value. It generates and optionally saves a figure
...
def cyto3d(save_root, save_figure=True):
    """
    Generates and optionally saves a performance figure for 3D segmentation models.

    This method evaluates different 3D model architectures by calculating
    average precision scores based on ground truth masks and produced masks
    from each model. It visualizes the results by plotting these scores against
    IoU (Intersection over Union) thresholds, allowing for a comparative analysis
    of model performance.

    Args:
        save_root: The directory path where ground truth and model masks are stored.
        save_figure: A boolean flag indicating whether to save the generated figure.

    Returns:
        None: This method does not return any value. It produces a plot and
        can save it to the specified directory if requested.
    """
