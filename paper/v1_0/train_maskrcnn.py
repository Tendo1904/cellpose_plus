...
class NucleusInferenceConfig(NucleusConfig):
    """
    NucleusInferenceConfig is a configuration class for inference settings related to nucleus detection tasks.
    ...
    The class includes attributes that define the number of GPUs to use, the number of images to process per GPU, the method of resizing images, and the non-maximum suppression threshold for region proposal networks during inference.
    """
...
class NucleusDataset(utils.Dataset):
    ...
    NucleusDataset is a class for handling and processing a dataset of nuclei images for segmentation tasks. It provides methods to load images and their associated instance masks.
    ...
    The methods enable loading a specific subset of the nuclei dataset and generating instance masks for the images, while the attributes hold the root directory of the dataset and the specific subset being used.
    """
...
def compute_batch_ap(dataset, image_ids, verbose=1):
    ...
    This method loads images from the specified dataset, runs object detection on each image,
    and computes the Average Precision over a range of IoU thresholds (0.5 to 0.95) for the
    detected objects. The AP values are returned for further analysis.

    Args:
        dataset: The dataset from which to load images and ground truth data.
        image_ids: A list of image identifiers for which to compute the Average Precision.
        verbose: A flag to control verbosity of output; if set to 1, prints out AP information.

    Returns:
        A list containing the Average Precision values computed for each image in the batch.
    """
    APs = []
    for image_id in image_ids:
        # Load image
