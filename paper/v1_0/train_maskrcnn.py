...
class NucleusInferenceConfig(NucleusConfig):
    ...
    NucleusInferenceConfig is a configuration class for performing nucleus inference tasks. It encapsulates settings and parameters necessary for configuring the inference process.
    ...
    This class allows users to customize the inference process by adjusting the number of GPUs, the number of images processed per GPU, the resizing mode for images, and the non-maximum suppression threshold used in Region Proposal Networks.
    """
...
class NucleusDataset(utils.Dataset):
    """
    NucleusDataset is a class for handling the nuclei dataset, providing functionalities to load dataset subsets and generate instance masks for images.

    Attributes:
    - dataset_dir: The root directory where the dataset is stored.
    - subset: The specific subset of the dataset that is currently loaded.
    ...
    The methods allow users to load different segments of the dataset and process images to retrieve masks and class IDs, facilitating tasks related to instance segmentation in images.
    """
...
def compute_batch_ap(dataset, image_ids, verbose=1):
    """
    Computes the average precision (AP) for a batch of images.

    This method evaluates the object detection performance on a dataset by calculating the average precision
    for each image in the specified list of image IDs. The AP is computed over a range of intersection over union (IoU) thresholds.

    Args:
        dataset: The dataset containing images and their corresponding ground truth data.
        image_ids: A list of image identifiers for which the AP is to be computed.
        verbose: An integer flag to control the verbosity of the output. If set to 1, additional information will be printed.

    Returns:
        A list of average precision values for each image in the batch.
    """
    APs = []
    for image_id in image_ids:
        # Load image
