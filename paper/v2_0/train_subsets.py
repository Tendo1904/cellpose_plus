...
def train_model(
    ...
    """
    Trains a deep learning model for image segmentation using specified parameters.

    This method allows users to train either a Cellpose or Unet model based on the provided configuration. It handles data loading, model initialization, training process, and optional evaluation of the trained model.

    Args:
        root: Path to the directory containing training data.
        model_type: Type of model to use ('cellpose' or 'unet').
        cell_type: Type of cells to segment (optional).
        tissue_type: Type of tissue for segmentation (optional).
        platform_type: Type of platform used for the images (optional).
        full_img: Flag to indicate whether to use full images or patches.
        ntrain: Number of training samples or fraction of the dataset to use for training.
        ntest: Number of test samples or fraction of the dataset to use for testing.
        weight_decay: Regularization parameter to apply during training.
        n_epochs: Number of epochs for training the model.
        learning_rate: Initial learning rate for the optimizer.
        batch_size: Number of samples processed before the model is updated.
        nimg_per_epoch: Number of images processed per epoch.
        start_pretrained: Flag to indicate whether to start training from a pretrained model.
        run_eval: Flag to enable evaluation after training.
        save_results: Flag to save the training results.
        save_test_masks: Flag to save predicted masks for test images.
        seed: Random seed for reproducibility.
        flow_threshold: Threshold for flow during evaluation.
        pretrained_model: Path to a pretrained model file (optional).

    Returns:
        The path where the trained model has been saved.
    """
...
def eval_model(
    ...
    """
    Evaluate a model on test data and generate performance metrics.

    Parameters:
        model_path: Path to the pretrained model.
        diam_mean: Mean diameter of the objects to be detected.
        nmasks: Number of masks in the training dataset.
        root: Root directory for input and output files.
        model_type: Type of the model to evaluate (default is 'cellpose').
        cell_type: Optional specification for the type of cell if using cellpose.
        tissue_type: Optional specification for the type of tissue if using tissue net.
        platform_type: Optional specification for the platform type if required.
        ntest: Number of test images to evaluate (default is 0).
        flow_threshold: The threshold for flow to consider (default is 0.4).
        save_results: Flag to indicate if results should be saved (default is False).
        save_test_masks: Flag to indicate if test masks should be saved (default is False).

    Returns:
        A tuple containing test data, test masks, predicted masks, and calculated metrics
        (average precision, true positives, false positives, false negatives).
    """
...
def eval_model_cyto(
    ...
    """
    Evaluates a model for cell segmentation using the specified parameters.

    This method loads training data and corresponding masks, computes mean diameters
    for the training masks, and invokes the evaluation of the model based on the
    provided model type and other configuration settings.

    Args:
        root: The root directory containing the training data.
        model_type: The type of model to use for evaluation (default is 'cellpose').
        cell_type: The specific type of cells to evaluate.
        tissue_type: The type of tissue to consider during evaluation.
        platform_type: The platform type of the data.
        ntest: The number of test samples to evaluate (default is 0).
        flow_threshold: The flow threshold used in model evaluation (default is 0.4).
    ...
    Returns:
        None: This method does not return any value, but it may save evaluation results based on the flags.
    """
    if model_type == "cellpose":
