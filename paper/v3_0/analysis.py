...
def blind_denoising(folder):
    """
    Performs blind denoising on images located in the specified folder.

        This method applies two denoising algorithms, noise2void and noise2self, to images
        in the provided folder that correspond to different cellular types (cyto2 and nuclei).
        It evaluates the performance of the denoising using average precision metrics.

        Args:
            folder: The path to the folder containing image data, expected to have subdirectories
                    for different cellular types.

        Returns:
            None: This method does not return any value; it prints the average precision scores
                  for denoised masks generated by both algorithms.
    """
    ctypes = ["cyto2", "nuclei"]
    for ctype in ctypes:
...
def real_examples_ribo(root):
    """
    Process ribosomal images to extract segmentation masks and average precision metrics.

        This method reads and processes noisy image data from the specified directory, applies
        denoising models, segments the images, and computes average precision for different
        segmentation methods. The results, including average precision metrics and segmentation
        masks, are saved to .npy files for further analysis.

        Args:
            root: The directory path where the image files are stored.
...
def cyto3_comparisons(folder):
    ...
    for net_type in net_types[-1:]:
        if net_type == "generalist":
            ...
            seg_model = models.CellposeModel(
                gpu=True, backbone="transformer", pretrained_model=pretrained_model
            )
        for f in folders:
