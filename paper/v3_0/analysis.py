...
def blind_denoising(folder):
    """
    Denoises images using two different methods: noise2void and noise2self.

    This method iterates through different cell types and applies both
    the noise2void and noise2self denoising techniques on the images located
    in the specified folder. It computes the average precision for both
    denoising methods by comparing the predicted masks to the true masks
    and prints the results.

    Args:
        folder: The path to the folder containing images and noisy test data.

    Returns:
        None: This method does not return a value but prints the average precision
        for both denoising methods.
    """
    ctypes = ["cyto2", "nuclei"]
    for ctype in ctypes:
...
def real_examples_ribo(root):
    """
    Run the Cellpose segmentation and denoising models on real example images, processing noisy images and saving results.

    Args:
        root: The root directory containing input image files for denoising and segmentation.

    Returns:
        None: This method does not return a value. It saves processed data and results to NumPy files in the specified root directory.
    """
