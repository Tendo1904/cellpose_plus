...
def blind_denoising(folder):
    """
    Performs blind denoising of images located in a specified folder using two methods:
    noise2void and noise2self. For each method, it trains models per image, evaluates the denoised
    results against ground truth masks, and prints the average precision metrics.

    Args:
        folder: The path to the folder containing image subdirectories named
                'images_cyto2' and 'images_nuclei', as well as a
                'noisy_test' subdirectory with the test data.
...
def real_examples_ribo(root):
    """
    Processes and analyzes ribosomal images by denoising them and computing average precision scores for different noise reduction models.

    Args:
        root: The directory path where the image files are located.

    Returns:
        None: This method saves the processed data to .npy files in the specified directory.
    """
