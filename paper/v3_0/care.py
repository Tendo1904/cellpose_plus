...
def CIL_dataset(root):
    """
    Create a dataset from images in the Cell Image Library for training and validation.

    This method reads training and test images from specified directories, processes them,
    adds noise, and then saves the resulting images and corresponding ground truth images to
    new directories. The training images are divided into training and validation sets.

    Args:
        root: The root directory containing the 'train' and 'test' subdirectories.
...
def train_test_specialist(root, lr=0.001, n_epochs=100, test=True):
    """
    Trains a specialist model using provided noisy data and evaluates its performance.

    Args:
        root: The root directory containing the dataset.
        lr: Learning rate for the training process.
        n_epochs: Number of training epochs.
        test: Flag indicating whether to return minimum validation loss or to perform testing.
    ...
    Returns:
        ...
        and the average precision metrics computed for the evaluation.
    """
