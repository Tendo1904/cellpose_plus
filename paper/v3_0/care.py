...
def CIL_dataset(root):
    """
    CIL_dataset processes microscopy images from a specified root directory, generating training and validation datasets.

    This method reads images from the given root path, normalizes them, adds noise, and divides them into training and validation sets. The processed images and their corresponding ground truths are saved to disk in specified directories.

    Args:
        root: The root directory containing the 'train' and 'test' image folders.

    Returns:
        None: The method saves the processed images to disk and returns no value.
    """
...
def train_test_specialist(root, lr=0.001, n_epochs=100, test=True):
    """
    Trains a model to restore images and optionally evaluates it on test data.

    This method handles the training of a machine learning model using raw image data,
    configures the model settings, and provides functionality to test the model if requested.

    Args:
        root: The directory path that contains the training and test dataset.
        lr: The learning rate for the model training process.
        n_epochs: The number of epochs to train the model.
        test: A boolean indicating whether to evaluate the model on test data.

    Returns:
        If test is False, returns the minimum validation loss observed during training.
        If test is True, returns a tuple containing the restored images, the masks generated
        from the model's predictions, and the average precision scores calculated from the test results.
    """
