...
def load_benchmarks(
    ...
    """
    Load benchmark data for image processing and evaluate metrics.

        This method retrieves benchmark datasets related to image processing and
        calculates average precision and other evaluation metrics based on the
        loaded data. It supports different noise types and allows for the inclusion
        of additional datasets if specified.

        Args:
            folder: The directory path where the benchmark images and masks are stored.
            noise_type: The type of noise to simulate in the test images (default is "poisson").
            ctype: The classification type which determines the dataset to load (default is "cyto2").
            thresholds: An array of thresholds to use for calculating average precision (default is a range from 0.5 to 1.0).
            others: A boolean indicating whether to include additional datasets in the load process (default is True).

        Returns:
            A tuple containing:
                - aps: An array of average precision scores for different predictions.
                - tps: An array of true positives for different predictions.
                - fps: An array of false positives for different predictions.
                - fns: An array of false negatives for different predictions.
                - imgs_all: A list containing all the test images loaded.
                - noise_levels: An array of noise levels corresponding to the test images.
                - masks_all: A list containing all the ground truth and predicted masks loaded.
                - masks_true: The true masks for the test data.
                - diam_test: The diameter values for the test images.
                - flows_perseg: The flow data for the per-segmentation if available.
                - flows_true: The true flow data if available.
    """
...
def draw_net(fig, ax, arrow_in=True, arrow_out=True):
    """
    Draws a simple representation of a network diagram.

        This method creates a visual representation of a network using the provided figure
        and axes. The network consists of annotated connections, with optional arrows indicating
        input and output directions.

        Args:
            fig: The figure object that the axes belong to.
            ax: The axes object where the network will be drawn.
            arrow_in: A boolean flag to indicate whether to draw an input arrow. Defaults to True.
            arrow_out: A boolean flag to indicate whether to draw an output arrow. Defaults to True.
...
def fig1(folder, save_fig=False):
    """
    Generate a figure that visualizes benchmarks for image denoising and segmentation.

        This method creates a comprehensive visualization by loading benchmarks from a specified folder,
        processing the data, and generating various plots including average precision metrics and image
        comparisons. The visualizations assist in analyzing the performance of denoising and segmentation
        algorithms.

        Args:
            folder: The directory path where benchmark data is stored.
            save_fig: A boolean flag indicating whether to save the generated figure as a PDF file.
...
def denoising_ex(
    ...
    """
    Generates denoising or deblurring examples based on input images and parameters.

        This method processes a set of images from a specified folder, applies noise reduction
        techniques according to the specified noise type, and produces a set of visual examples
        for evaluation. It supports different types of noise and produces plots demonstrating the
        effectiveness of the denoising methods applied.

        Args:
            folder: The directory path containing the input image data.
            grid: The grid layout for the output plots.
            il: An integer value for indexing the plots.
            transl: A translation parameter for plotting.
            kk: A list of integers representing the indices of the images to process (default is [0, 1, 5]).
            seg: A list of integers indicating segmentation options (default is [0, 0]).
            noise_type: A string specifying the type of noise ('poisson', 'blur', or 'downsample').
            ctype: A string that defines the type of cell analysis ('cyto2' or others).
            j0: An integer indicating the starting index for the grid layout.
            diams: A list of diameters for images; if None, defaults to a preset.
            dy: An integer that indicates an offset for the vertical position of subplots.

        Returns:
            An updated index for plotting images after generating the denoising examples.
    """
...
def suppfig_seg(folder, save_fig=False):
    """
    Generates supplemental figures for segmentation analysis.

        This method loads benchmark data related to cytoplasmic segmentation and
        visualizes the performance metrics, including false positive rate, true
        positive rate, and false negative rate. The results are plotted in a
        grid layout for comprehensive analysis. It can also save the generated figures
        to a specified directory.

        Args:
            folder: The directory where benchmark data files are stored.
            save_fig: A boolean indicating whether to save the generated figure as a PDF.

        Returns:
            None: This method does not return any value. It only produces visual
            outputs and can optionally save them to a file.
    """
    thresholds = np.arange(0.5, 1.05, 0.05)
...
def suppfig_nuclei(folder, save_fig=False):
    """
    Generates supplementary figures for nuclei analysis.

        This method loads benchmark data for nuclei detection and plots false positive rates,
        true positive rates, and false negative rates across different thresholds. It organizes
        the figures into a grid layout and can save the resulting figure as a PDF.

        Args:
            folder: The directory where the benchmark data is stored.
            save_fig: A boolean indicating whether to save the generated figure as a PDF.
...
def fig2(
    ...
    """
    Generate and display a detailed figure comparing segmentation performance for various imaging conditions.

        This method creates a multi-panel figure that shows images and segmentation results
        for both noisy and denoised data. It compares performance metrics such as average precision
        at different Intersection over Union (IoU) thresholds, and includes visualizations of ground truth
        and predicted segmentations. The figure can be optionally saved to disk.

        Args:
            folder: The directory containing the benchmark data.
            folder2: The path to the folder containing calculated masks data (default is set to a specific path).
            folder3: The path to the directory containing ribo denoise data (default is set to a specific path).
            save_fig: A boolean indicating whether to save the generated figure to disk (default is False).

        Returns:
            None: This method displays a figure and has the option to save it as a PDF file.
    """
    thresholds = np.arange(0.5, 1.05, 0.05)
...
def fig3(
    ...
    """
    Generates and saves a figure illustrating segmentation performance metrics.

        This method creates a multi-panel figure that visualizes average precision (AP)
        across different Intersection over Union (IoU) thresholds and compares segmentation
        results from noisy, clean, and denoised images. It utilizes the specified folder to
        load benchmark data and optional configurations for visualization.

        Args:
            folder: The directory path that contains the benchmark data for segmentation.
            folder2: An optional directory path for additional data files,
                      defaults to a predefined test data location.
            save_fig: A boolean flag indicating whether to save the generated figure as a PDF.
                      Defaults to False.

        Returns:
            None: This method does not return any value but produces and optionally saves a figure.
    """
    thresholds = np.arange(0.5, 1.05, 0.05)
...
def fig4(folder, ctype="cyto2", save_fig=False):
    """
    Generate and save a figure depicting performance metrics for image analysis.

        This method creates a plot that visualizes the average precision (AP) at various Intersection over Union (IoU) thresholds for images analyzed under different noise conditions. The figure is designed for comparison between performance metrics related to blurry images and bilinearly upsampled images.

        Args:
            folder: The directory containing the input data for analysis.
            ctype: The type of analysis to be performed, default is "cyto2".
            save_fig: A flag indicating whether to save the generated figure as a file.

        Returns:
            None: The method does not return a value but may save a figure to disk if requested.
    """
    titlesd = np.array(titles)[[0, 1, 4, 5, 6, 7]]
    colsd = np.array(cols)[[0, 1, 4, 5, 6, 7]]
    titlesd[0] = "clean"
...
def load_benchmarks_specialist(folder, thresholds=np.arange(0.5, 1.05, 0.05)):
    """
    Load benchmark data for noise reduction specialists and evaluate metrics.

        This method loads various noisy test datasets from a specified folder, processes
        the image and mask data, and computes performance metrics such as average precision,
        true positives, false positives, and false negatives. The results are returned in a
        structured format for further analysis.

        Args:
            folder: The path to the folder containing the test images.
            thresholds: The thresholds to use for evaluating average precision (optional).

        Returns:
            A tuple containing:
                - An array of average precision values for the different models.
                - An array of true positive counts.
                - An array of false positive counts.
                - An array of false negative counts.
                - A list of test images from the different models.
                - An array of noise levels corresponding to the images.
                - A list of masks for all models.
                - The ground truth masks.
                - An array of diameters used in the tests.
    """
    noise_type = "poisson"
    ctype = "cyto2"
    nimg_test = 11
...
def suppfig_specialist(folder, save_fig=True):
    """
    Generates and optionally saves diagnostic figures for specialist dataset evaluations.

        This method reads training and ground truth images from the specified folder,
        computes various metrics, and produces a series of plots to visualize the results
        of image evaluations based on different thresholds. The generated figures can be
        saved as a PDF file.

        Args:
            folder: The path to the folder containing the image datasets.
            save_fig: Flag indicating whether to save the generated figures. Defaults to True.

        Returns:
            None: This method does not return any value. It visualizes and saves figures if requested.
    """
    n_train = 100
    im_train = [
        io.imread(
...
def suppfig_impr(folder, save_fig=True):
    """
    Generates and saves supplementary figures comparing different noise types in image datasets.

        This method processes images and corresponding masks from a specified folder, computes average precision
        metrics for various noise types, and generates figures that visualize the performance of denoising and deblurring
        techniques across different image categories. It also allows for saving the generated figures to a file.

        Args:
            folder: The directory containing the image and mask data folders.
            save_fig: A boolean flag indicating whether to save the generated figures as a PDF. Defaults to True.
...
def load_benchmarks_generalist(
    ...
    """
    Loads benchmark data for generalist models.

        This method loads test images, noisy images, and their corresponding masks from a specified
        folder. It also retrieves additional noise-related data and computes various metrics related
        to the masks.

        Args:
            folder: The base directory containing the image data.
            noise_type: The type of noise applied to the images (default is "poisson").
            ctype: A string representing the cell type for loading specific data configurations
                   (default is "cyto2").
            thresholds: A sequence of thresholds used for computing metrics (default is
                        np.arange(0.5, 1.05, 0.05)).

        Returns:
            A tuple containing:
                - aps: Average precision scores for the masks.
                - tps: True positive counts for each mask.
                - fps: False positive counts for each mask.
                - fns: False negative counts for each mask.
                - imgs_all: A list of all loaded images.
                - noise_levels: A list of noise levels corresponding to the test images.
                - masks_all: A list of all loaded masks.
                - masks_true: The ground truth masks.
                - diam_test: The diameter of the test images.
    """
    nimg_test = 68 if ctype == "cyto2" else 111
    folder_name = ctype
    root = Path(f"{folder}/images_{folder_name}/")
...
def fig6(folder, save_fig=True):
    """
    Generates and saves a figure comparing performance metrics of various image restoration models.

        This method processes images from specified folders, applies denoising and segmentation models,
        computes average precision (AP) metrics for different noise types, and visualizes the results in a
        structured figure layout. The figure can also be saved to a file.

        Args:
            folder: The directory containing the image mask data files.
            save_fig: A boolean flag indicating whether to save the generated figure to a file.
                      Defaults to True.

        Returns:
            None: This method does not return any value. It produces a figure and saves it if requested.
    """
    folders = [
        "cyto2",
        "nuclei",
...
def suppfig_generalist_examples(folder, save_fig=True):
    """
    Generates supplementary figures for generalist image examples and saves them.

        This method processes various images and their associated segmentation masks, applies noise and denoising transformations, and generates visual comparisons of the results. The figures are organized in a grid format, displaying clean, noisy, and denoised images, along with their corresponding masks.

        Args:
            folder: The directory containing the input mask files.
            save_fig: A boolean flag indicating whether to save the generated figures to the disk.

        Returns:
            None: The method does not return any value but generates and optionally saves figures.
    """
    cols0 = np.array(
        [
            [0, 0, 0],
...
def generalist_restoration_panels(
    ...
    """
    Generates restoration panels for generalist image processing.

        This function visualizes output images alongside their corresponding labels and masks
        in a grid layout, allowing for easy comparison and analysis of restoration results.

        Args:
            fig: The figure object to which the subplots will be added.
            grid: The grid specifications for subplot arrangement.
            imgs: A list of images to be displayed in the panels.
            lbls: A list of label masks associated with the images.
            masks: A list of binary masks used for highlighting in the panels.
            diams: A list of diameters used for scaling the images appropriately.
            api: A matrix of average precision values to be displayed in the panels.
            titlesj: Titles for the image categories to be displayed in the panels.
            colsj: Colors associated with the titles for visual distinction.
            titlesi: Titles for each individual image.
            j0: The starting index for the subplot grid to allow flexibility in layout (default is 0).
            ly0: The initial y-axis limit for the subplot configuration (default is 250).
            letter: A boolean indicating whether to apply letter formatting (default is False).
            il: An index for tracking label placement during layout adjustments (default is 0).

        Returns:
            None: This function modifies the figure in place and returns nothing.
    """
    if letter:
        il = j0 > 0
...
def load_seg_generalist(folder):
    """
    Loads segmentation data for generalist models from specified folders.

        This method processes segmentation results from various model types
        and the corresponding ground truth and predicted masks. It aggregates
        performance metrics and retrieves images and masks based on predefined
        indexes.

        Args:
            folder: The path to the directory containing the segmentation
            mask files for different models.

        Returns:
            A tuple containing:
                - apcs: A NumPy array of average precision scores for each
                  model type across the specified folders.
                - api: A list of average precision scores for the generalist
                  model from selected images.
                - imgs: A list of images corresponding to the selected
                  indexes.
                - masks_true: A list of ground truth masks corresponding to
                  the selected images.
                - masks_pred: A list of predicted masks corresponding to
                  the selected images.
    """
    folders = [
        "cyto2",
...
def fig5(folder, save_fig=True):
    """
    Generate and save a figure for segmentation performance analysis.

        This method creates a visual representation of segmentation performance metrics
        based on various segmentation models. It consists of subplots showcasing the
        average precision at different Intersection over Union (IoU) thresholds,
        along with the original and predicted masks for each model.

        Args:
            folder: The directory from which segmentation data will be loaded.
...
def old_nuclei_ex(folder, save_fig=False):
    """
    Visualizes and optionally saves benchmark results for nuclei segmentation.

        This function processes benchmark data related to nuclei detection and
        segmentation, generating visualizations that display various metrics such
        as average precision (AP) and overlays of ground truth versus predicted
        segmentations. It allows for the option to save the generated figures for
        future reference.

        Args:
            folder: The directory containing benchmark data files.
            save_fig: A flag indicating whether to save the generated figures to
                disk. Defaults to False.

        Returns:
            None: This method does not return any value. It generates visualizations
            and optionally saves the figure files.
    """
    out = load_benchmarks(folder, ctype="nuclei", thresholds=thresholds)
    (
        aps,
