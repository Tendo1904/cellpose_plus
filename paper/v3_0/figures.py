...
def draw_net(fig, ax, arrow_in=True, arrow_out=True):
    """
    Draws a network diagram on the provided axes.

    This method creates a simple visualization of a network structure using
    annotations and arrows to represent connections. It places nodes in predefined
    locations and optionally adds arrows indicating input and output.

    Args:
        fig: The figure object to which axes are added for drawing the network.
        ax: The axes object on which the network will be drawn.
        arrow_in: A boolean indicating whether to draw an input arrow. Defaults to True.
        arrow_out: A boolean indicating whether to draw an output arrow. Defaults to True.

    Returns:
        None: This method does not return a value. It directly modifies the provided axes.
    """
...
def fig1(folder, save_fig=False):
    """
    Generates a figure based on benchmark results visualizing various images and metrics.

    Args:
        folder: The directory containing benchmark data to be loaded.
        save_fig: A flag indicating whether to save the generated figure to a file.
...
def denoising_ex(
    ...
    """
    Displays denoising examples for images, allowing visualization of the effects of various noise types and denoising techniques on image data.

    Args:
        folder: The directory containing image and mask files.
        grid: The grid layout for displaying images.
        il: An integer for tracking the index of the subplot.
        transl: A translation parameter for labeling.
        kk: A list of indices related to the images to display.
        seg: A list indicating whether to segment images or not.
        noise_type: The type of noise present in the images (e.g., "poisson", "blur", "downsample").
        ctype: Specifies the type of images being processed (e.g., "cyto2", "nuclei").
        j0: An index for adjusting the layout of the plots.
        diams: Optional list of diameters for normalization purposes.
        dy: A vertical displacement parameter for subplot positioning.

    Returns:
        An integer representing the updated index for subplots.
    """
...
def suppfig_seg(folder, save_fig=False):
    """
    Generates and optionally saves supplementary figures for benchmarking cellular segmentation.

    Args:
        folder: The directory containing the data to be analyzed.
        save_fig: A boolean flag indicating whether to save the generated figures.
...
def suppfig_nuclei(folder, save_fig=False):
    ...
    This method creates a figure displaying various performance metrics related to nuclei detection,
    including false positive rates, true positive rates, and false negative rates. It utilizes specified
    thresholds to load benchmark data and visualizes the results in a structured grid layout. The figure
    can also be saved to a file if specified.

    Args:
        folder: The directory containing the benchmark data for nuclei analysis.
        save_fig: A boolean indicating whether to save the generated figure to a file.

    Returns:
        None: This method does not return any value; it generates a figure and saves it if requested.
    """
    thresholds = np.arange(0.5, 1.05, 0.05)
...
def fig2(
    ...
    """
    Create and save a figure comparing segmentation performance for various denoising methods.

    Args:
        folder: The path to the folder containing benchmark data.
        folder2: The path to the folder containing cellpose masks (default is "/media/carsen/ssd4/denoising/Projection_Flywing/test_data").
        folder3: The path to the folder containing ribo denoising data (default is "/media/carsen/ssd4/denoising/ribo_denoise/").
        save_fig: A boolean flag indicating whether to save the figure as a PDF file (default is False).
...
def fig3(
    ...
    """
    Generates a figure displaying segmentation performance metrics and sample images from the provided data folders.

    Args:
        folder: The directory containing benchmark data for the analysis.
        folder2: An optional directory for loading additional test images and masks. Defaults to a predefined path.
        save_fig: A boolean indicating whether to save the generated figure to a file. Defaults to False.
...
def fig4(folder, ctype="cyto2", save_fig=False):
    """
    Generates a figure comparing performance metrics based on different noise types.

    Args:
        folder: The directory containing the benchmark data for analysis.
        ctype: The type of analysis to perform, defaulting to "cyto2".
...
def load_benchmarks_specialist(folder, thresholds=np.arange(0.5, 1.05, 0.05)):
    """
    Loads benchmark data for various noise types and computes performance metrics.

    This method loads test images and corresponding masks from specified folders
    and computes average precision, true positives, false positives, and false negatives
    for each set of masks against the true masks. Additionally, it returns the test images
    and noise levels for further analysis.

    Args:
        folder: The directory where the images and benchmark data are stored.
        thresholds: A range of thresholds for calculating precision metrics.

    Returns:
        A tuple containing:
            - Average precision scores for each mask set.
            - True positive counts for each mask set.
            - False positive counts for each mask set.
            - False negative counts for each mask set.
            - A list of all test images.
            - A list of noise levels associated with the test images.
            - A list of all masks.
            - The true masks used for benchmarking.
            - The diameters corresponding to the test images.
    """
    noise_type = "poisson"
    ctype = "cyto2"
    nimg_test = 11
...
def suppfig_specialist(folder, save_fig=True):
    """
    Generates and saves a supplementary figure for the specialist dataset, visualizing various images and metrics.

    Args:
        folder: The directory containing the dataset files.
        save_fig: A flag indicating whether to save the generated figure as a PDF.
...
def suppfig_impr(folder, save_fig=True):
    """
    Generates supplementary figures to demonstrate improvements in image processing techniques.

    Args:
        folder: The path to the folder containing image data.
        save_fig: Indicates whether to save the generated figure as a PDF. Defaults to True.

    Returns:
        None: The method does not return any value, but saves a figure showing the results.
    """
    aps_all = [[], []]
    imgs_all, masks_all = [[], []], [[], []]
    inds_all = [[], []]
...
def load_benchmarks_generalist(
    ...
    """
    Loads benchmark images and masks for generalist testing, calculates average precision metrics and returns the results.

    Args:
        folder: The path to the folder containing image data.
        noise_type: The type of noise applied to the images (default is "poisson").
        ctype: The classification type used to select the dataset (default is "cyto2").
        thresholds: An array of thresholds for average precision calculation (default is an array from 0.5 to 1.0 with a step of 0.05).

    Returns:
        A tuple containing:
            - aps: Array of average precision scores for each mask.
            - tps: Array of true positives counts for each mask.
            - fps: Array of false positives counts for each mask.
            - fns: Array of false negatives counts for each mask.
            - imgs_all: List of all test images (noisy and original).
            - noise_levels: Array of noise levels used in the dataset.
            - masks_all: List of all masks (true and noisy).
            - masks_true: Array of true masks.
...
def fig6(folder, save_fig=True):
    """
    Generate and optionally save a figure (fig6) that represents the performance of segmentation models
    on different types of images after applying various noise types.

    Args:
        folder: The path to the folder containing the necessary data files.
...
def suppfig_generalist_examples(folder, save_fig=True):
    """
    Generates and saves example figures for generalist models applied to image datasets.

    Args:
        folder: The path to the directory containing the dataset files.
        save_fig: A boolean indicating whether to save the generated figure.
...
def generalist_restoration_panels(
    ...
    """
    Generates and displays restoration panels for a series of images, labels, and masks.

    This method creates a grid of subplots that visualize images along with their corresponding ground truth labels and masks. It formats the display based on the specified parameters and enhances the visual presentation with outlines and additional text annotations.

    Args:
        fig: The figure object used to create the subplots for the visualization.
        grid: A grid layout specification that determines the arrangement of subplots.
        imgs: A list of image arrays to be displayed in the panels.
        lbls: A list of label arrays corresponding to the images.
        masks: A list of mask arrays to be overlaid on the images for visualization.
        diams: A list of diameters related to the labels, used for scaling.
        api: A two-dimensional array containing performance metrics to be displayed on the panels.
        titlesj: A list of titles for the main rows of the grid.
        colsj: A list of colors corresponding to the titles for better visibility.
        titlesi: A list of titles for each individual panel.
        j0: An offset index for the grid starting position (default is 0).
        ly0: A scaling factor for the subplot height based on the diameter (default is 250).
        letter: A boolean flag indicating whether to use a letter-based annotation format (default is False).
        il: An integer index for tracking label placement or annotation (default is 0).

    Returns:
        None: This method displays a visualization and does not return a value.
    """
    if letter:
        il = j0 > 0
...
def load_seg_generalist(folder):
    """
    Load segmentation masks and performance metrics from specified folders.

    This method loads performance metrics and associated images and masks from
    a set of predefined folders based on the segmentation model type. It aggregates
    the results for the "generalist" model while also handling exceptions during the
    loading process. The method returns arrays related to average precision,
    images, true masks, and predicted masks across different fold types.

    Args:
        folder: The directory containing the folders with segmentation mask data.

    Returns:
        A tuple containing:
            - apcs: A numpy array of average precision values across network types and folders.
            - api: An array of average precision indices for the generalist model.
            - imgs: A list of images associated with the generalist model.
            - masks_true: A list of true masks corresponding to the loaded images.
            - masks_pred: A list of predicted masks corresponding to the loaded images.
    """
    folders = [
        "cyto2",
        "nuclei",
...
def fig5(folder, save_fig=True):
    """
    Generates a figure (Figure 5) displaying segmentation performance metrics for various models.

    This method creates a grid of subplots, where each subplot visualizes the segmentation results for a specific model. It also generates average precision plots based on intersection over union (IoU) thresholds.

    Args:
        folder: The directory path where segmentation data is stored.
...
def old_nuclei_ex(folder, save_fig=False):
    """
    Generates and visualizes benchmarking results for nucleus segmentation, including average precision metrics and overlaying segmentation outlines.

    Args:
        folder: The directory containing the input data for benchmarking.
        save_fig: A boolean flag indicating whether to save the generated figures to a file.
