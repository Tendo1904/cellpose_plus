...
def load_benchmarks(
    ...
    """
    Loads benchmark images and corresponding masks from a specified folder for a given noise type and cell type.

    This method retrieves test images and masks for evaluation of various noise levels and types. It supports optional retrieval of additional datasets for benchmarking. The method also computes evaluation metrics such as average precision, true positives, false positives, and false negatives.

    Args:
        folder: The path to the folder containing the dataset.
        noise_type: The type of noise to be applied to the images. Default is "poisson".
        ctype: The cell type from which to load images. Default is "cyto2".
        thresholds: An array of thresholds used for evaluating performance metrics. Default is an array from 0.5 to 1.0 with a step of 0.05.
        others: A boolean flag indicating whether to load additional datasets. Default is True.

    Returns:
        A tuple containing:
            - aps: Average precision values for each mask.
            - tps: True positives for each mask.
            - fps: False positives for each mask.
            - fns: False negatives for each mask.
            - imgs_all: A list of all loaded images.
            - noise_levels: The noise levels corresponding to the noisy test images.
            - masks_all: A list of all loaded masks.
            - masks_true: The ground truth masks.
            - diam_test: The diameter profile for the test images.
            - flows_perseg: Flows associated with the segmented images, if available.
            - flows_true: True flows, if available.
    """
...
def fig1(folder, save_fig=False):
    """
    Generates and saves a figure based on benchmark results for image processing models.

    This method loads benchmark data from the specified folder, processes the results, and generates
    visualizations that illustrate the performance of different image processing models under various
    conditions, including images with noise and their corresponding denoised outputs.

    Args:
        folder: The directory path from which to load benchmark data.
        save_fig: A boolean flag that determines whether to save the generated figure to a file.
...
def denoising_ex(
    ...
    """
    Generates denoising examples for a specified type of noise.

    This method loads benchmark data from the specified folder and creates visual examples
    of denoised images based on the chosen noise type and other parameters. It plots the images
    and their denoised versions, along with associated metrics for visualization.

    Args:
        folder: The path to the directory containing the benchmark data.
        grid: The grid layout for plotting the images.
        il: The current index for storing the plot state.
        transl: Translation parameters for plot labels.
        kk: List of indices for selecting images to showcase (default is [0, 1, 5]).
        seg: List to indicate segmentation options (default is [0, 0]).
        noise_type: The type of noise to process, e.g., "poisson", "blur", or "downsample".
        ctype: The cell type for processing, e.g., "cyto2".
        j0: An additional parameter for subplot indexing.
        diams: Optional diameters for scaling the images.
        dy: Vertical displacement for subplot adjustments.

    Returns:
        The updated index for the plot state after processing the images.
    """
...
def suppfig_seg(folder, save_fig=False):
    """
    Generates and saves supplementary figures based on segmentation results from provided image folder.

    Args:
        folder: The directory containing the benchmark data and images for processing.
        save_fig: A boolean indicating whether to save the generated figure as a PDF.

    Returns:
        None: The method does not return any value. It either generates and displays the plot or saves it to a file if specified.
    """
    thresholds = np.arange(0.5, 1.05, 0.05)
...
def suppfig_nuclei(folder, save_fig=False):
    """
    Generate supplementary figures for nuclei analysis.

    This method loads benchmark data for nuclei and creates visualizations of false positive, true positive, and false negative rates. It can also save the generated figure as a PDF file.

    Args:
        folder: The directory from which to load the benchmark data.
...
def fig2(
    ...
    """
    Generates and saves a figure comparing segmentation performance using different methods.

    Parameters:
        folder: The directory containing benchmark files.
        folder2: The directory containing cp_masks.npy and other mask data.
        folder3: The directory containing ribo_denoise files for additional performance metrics.
        save_fig: A boolean flag indicating whether to save the generated figure.
...
def fig3(
    ...
    """
    Generates and optionally saves a figure visualizing segmentation performance metrics for denoised images.

    Args:
        folder: The directory containing benchmark data needed for generating the plots.
        folder2: The directory for loading additional data, defaults to a specified path.
...
def fig4(folder, ctype="cyto2", save_fig=False):
    """
    Generates a figure visualizing the average precision (AP) across different IoU thresholds
    for various denoising techniques applied to images.

    Args:
        folder: The directory containing the images and benchmarks to load.
        ctype: A string indicating the type of data used; defaults to "cyto2".
        save_fig: A boolean flag indicating whether to save the generated figure; defaults to False.
...
def load_benchmarks_specialist(folder, thresholds=np.arange(0.5, 1.05, 0.05)):
    """
    Loads benchmark images and corresponding masks from specified folder,
    calculates average precision metrics, and returns the results along with
    the images and noise levels.

    Args:
        folder: The directory containing the images and benchmark files.
        thresholds: An array of threshold values for calculating metrics.

    Returns:
        A tuple containing:
            - average_precision: Array of average precision values for each mask.
            - true_positives: Array of true positive counts for each mask.
            - false_positives: Array of false positive counts for each mask.
            - false_negatives: Array of false negative counts for each mask.
            - images: List of loaded images from the benchmark datasets.
            - noise_levels: Array of noise levels corresponding to the images.
            - masks: List of loaded masks from the benchmark datasets.
            - true_masks: Array of ground truth masks.
            - diameter: Array of diameters for the test images.
    """
    noise_type = "poisson"
    ctype = "cyto2"
    nimg_test = 11
...
def suppfig_specialist(folder, save_fig=True):
    """
    Generate and save a supplementary figure for specialist dataset analysis using images and benchmarks.

    Args:
        folder: The path to the folder containing the input images and ground truth data.
...
def suppfig_impr(folder, save_fig=True):
    """
    Generates supplementary figures to evaluate the improvement of image processing techniques
    using average precision metrics.

    Args:
        folder: The path to the folder containing the image data.
        save_fig: A boolean indicating whether to save the generated figure as a PDF file.
                  Defaults to True.

    Returns:
        None: The method does not return a value; it saves the figure to a specified location.
    """
    aps_all = [[], []]
    imgs_all, masks_all = [[], []], [[], []]
    inds_all = [[], []]
...
def load_benchmarks_generalist(
    ...
    """
    Loads benchmark data for generalist models from the specified folder and processes the images and masks according to the given parameters.

    Args:
        folder: The folder path containing the benchmark images and masks.
        noise_type: The type of noise added to the images, default is "poisson".
        ctype: The type of cell or context, default is "cyto2".
        thresholds: An array of thresholds used for calculating precision metrics, default is an array from 0.5 to 1.0 with step 0.05.

    Returns:
        A tuple containing:
            - aps: An array of average precision scores for each mask.
            - tps: An array of true positive counts for each mask.
            - fps: An array of false positive counts for each mask.
            - fns: An array of false negative counts for each mask.
            - imgs_all: A list of all test images loaded.
            - noise_levels: An array of noise levels associated with the test images.
            - masks_all: A list of all masks loaded, including true and noisy masks.
            - masks_true: An array of the true masks from the test dataset.
            - diam_test: An array representing the diameters of the test images.
    """
    nimg_test = 68 if ctype == "cyto2" else 111
    folder_name = ctype
    root = Path(f"{folder}/images_{folder_name}/")
...
def fig6(folder, save_fig=True):
    """
    Generates and saves a figure comparing different image restoration techniques on segmentation masks.

    Args:
        folder: The directory containing the dataset files.
        save_fig: A boolean flag indicating whether to save the generated figure as a PDF.

    Returns:
        None: This method does not return any value. It saves a figure to the specified directory if save_fig is True.
    """
    folders = [
        "cyto2",
        "nuclei",
...
def suppfig_generalist_examples(folder, save_fig=True):
    """
    Generates and saves supplementary figures showcasing generalist examples of image segmentation and denoising.

    Args:
        folder: The path to the directory containing the input mask files.
        save_fig: A boolean indicating whether to save the generated figure to a PDF file.
...
def generalist_restoration_panels(
    ...
    """
    Generates and displays generalist restoration panels for given images and labels.

    This method creates a visual representation of images and their corresponding labels, overlaid with outlines from masks, for comparison and analysis. It constructs a series of subplots based on the provided images and associated metadata.

    Args:
        fig: The figure object where the panels will be drawn.
        grid: The grid layout for placing subplots.
        imgs: A collection of images to be displayed.
        lbls: Corresponding labels for the images.
        masks: Masks used to outline regions in the images.
        diams: Diameters used for scaling.
        api: Average precision values to be displayed on the panels.
        titlesj: Titles to be used for different sections of the panels.
        colsj: Colors associated with the section titles.
        titlesi: Titles for individual images.
        j0: Index offset for subplots (default is 0).
        ly0: Base height for subplots (default is 250).
        letter: Boolean flag indicating whether to use letter format (default is False).
        il: Index to control label placement (default is 0).

    Returns:
        None: This method does not return any value. It directly modifies the provided figure to add the generated panels.
    """
    if letter:
        il = j0 > 0
...
def load_seg_generalist(folder):
    """
    Loads segmentation data for various network types from specified folders.

    This method retrieves and processes segmentation masks and corresponding performance data from files
    located in a given folder. It handles multiple network types and aggregates results for specific datasets.

    Args:
        folder: The path to the folder containing the mask files for each dataset.

    Returns:
        A tuple containing:
            - apcs: An array of average precision scores for each network type and dataset.
            - api: An array of average precision indices for the generalist network type.
            - imgs: A list of images corresponding to the generalist network type.
            - masks_true: A list of ground truth masks corresponding to the generalist network type.
            - masks_pred: A list of predicted masks corresponding to the generalist network type.
    """
    folders = [
        "cyto2",
        "nuclei",
...
def fig5(folder, save_fig=True):
    ...
    Generates and saves a figure comparing segmentation performances across different models.
...
def old_nuclei_ex(folder, save_fig=False):
    """
    Generate visualizations for nucleus segmentation metrics and denoising results.

    This method loads benchmark data for nucleus segmentation, creates visualizations for the
    average precision metrics, and provides a comparative view of segmentation results.

    Args:
        folder: The path to the folder containing benchmark data.
        save_fig: A boolean indicating whether to save the generated figures as a PDF file.
