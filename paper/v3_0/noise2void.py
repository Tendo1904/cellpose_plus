...
def train_per_image(img_noisy):
    """
    Train a model to denoise a single image using a trained framework, and predict the denoised output.

    Args:
        img_noisy: A noisy input image represented as a NumPy array.

    Returns:
        A denoised image as a NumPy array obtained from the trained model's predictions.
    """
    img = img_noisy.copy()
    print(img.shape)
    Ly, Lx = img.shape[-2:]
...
def train_per_image_synthetic(root, ctype="cyto2", plot=False, save=True):
    """
    Trains a model on noisy image data using the Cellpose segmentation framework and saves or plots the results.

    Args:
        root: The root directory where the input and output data is stored.
        ctype: The type of Cellpose model to be used for segmentation (default is "cyto2").
        plot: A boolean indicating whether to plot the intermediate results for each image (default is False).
        save: A boolean indicating whether to save the processed images and masks to disk (default is True).

    Returns:
        A tuple containing the processed images and their corresponding masks.
    """
    noise_type = "poisson"
...
def train_test_specialist(root, n_epochs=100, lr=5e-4, test=True):
    """
    Trains a specialist model on noisy image data or evaluates it based on provided test data.
    ...
    Args:
        ...
        test: A boolean flag indicating whether to perform testing after training (default is True).

    Returns:
        If test is False, returns the minimum validation loss achieved during training.
        If test is True, returns a tuple containing the predicted images, the predicted masks,
        and the average precision score across thresholds.
    """
    n_train = 3 * 89 * 20
    n_val = 89 * 20
