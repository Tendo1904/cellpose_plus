...
def train_per_image(img_noisy):
    """
    Trains a noise2void model on a given noisy image and returns the predicted denoised image.

    Args:
        img_noisy: The input noisy image from which to generate training and validation patches.

    Returns:
        The denoised image obtained from the trained model.
    """
    img = img_noisy.copy()
    print(img.shape)
    Ly, Lx = img.shape[-2:]
...
def train_per_image_synthetic(root, ctype="cyto2", plot=False, save=True):
    """
    Trains a model on synthetic noisy images for segmentation.

    This method processes a set of noisy test images and applies a training function to each image
    to generate segmentation masks. The resulting images and masks can optionally be plotted and saved.

    Args:
        root: The file path to the directory containing the noisy test images.
        ctype: The type of model to use for segmentation. Defaults to "cyto2".
        plot: A boolean flag indicating whether to plot the images and their segmentation masks. Defaults to False.
        save: A boolean flag indicating whether to save the resulting images and masks. Defaults to True.

    Returns:
        A tuple containing:
            - imgs_n2v: A list of synthesized images generated from the test noisy images.
            - masks_n2v: A list of segmentation masks corresponding to the synthesized images.
    """
    noise_type = "poisson"
...
def train_test_specialist(root, n_epochs=100, lr=5e-4, test=True):
    """
    Trains a model using noisy test images and optionally evaluates performance on test data.

    Args:
        root: The root directory containing the dataset.
        n_epochs: The number of training epochs (default is 100).
        lr: The learning rate for the training process (default is 5e-4).
        test: A boolean flag indicating whether to run the testing phase (default is True).

    Returns:
        The minimum validation loss if testing is disabled, otherwise returns a tuple containing:
        - imgs_n2v: The denoised images after the model's prediction.
        - masks_n2v: The predicted masks for the denoised images.
        - ap: The average precision scores calculated from the predicted masks.
    """
    n_train = 3 * 89 * 20
    n_val = 89 * 20
