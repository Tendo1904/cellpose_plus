...
def main():
    """
    Main function to execute the Cellpose training pipeline.

    This function sets up command line argument parsing for parameters, loads image data for training and testing,
    initializes the model, and starts the training process. It also handles the generation of random sub-sampling
    for training data based on the provided seed.

    Parameters:
        nsub: The number of training samples to be used (default is 0, meaning all samples).
        seed: The random seed for reproducibility (default is 0).
        n_epochs: The number of training epochs (default is 2000).
...
def save_results(folder, sroot):
    """
    Saves the results of cell segmentation metrics calculated using a pre-trained CellposeModel and DenoiseModel.

    This method evaluates the average precision of the segmentation results under different conditions
    and saves the results as numpy files. The metrics are calculated for various subsampling rates and
    the number of epochs used in the training of the denoising model.

    Args:
        folder: The directory where the images and results are stored.
        sroot: The root directory containing the pre-trained models for denoising.
