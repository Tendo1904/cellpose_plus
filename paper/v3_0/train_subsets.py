...
def main():
    """
    Main function to set up and run the Cellpose training process.

        This function initializes command line argument parsing for Cellpose parameters,
        prepares training and testing data, and sets up a model for denoising using the
        specified parameters. It handles loading images from a specified directory and
        normalizes them before feeding them into the training process.

        Parameters:
            nsub: Number of subsets of training data to use.
...
def save_results(folder, sroot):
    """
    Saves the average precision results of a cell segmentation model.

        This method evaluates the performance of a cell segmentation model using average precision at various
        thresholds and for different configurations. It processes images through a denoising model and applies
        a segmentation model to obtain masks, subsequently calculating average precision metrics.

        Args:
            folder: The path to the folder containing the images and model data.
            sroot: The root path for accessing pre-trained models and other resources.

        Returns:
            None: This method saves the results to numpy files and does not return any value.
    """
