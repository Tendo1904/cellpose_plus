...
def main():
    """
    Run the main processing routine for the Cellpose command line interface.

    This method initializes command line argument parsing, sets up the necessary
    data for training a denoising model, and executes the training process.
    It handles both training and testing data, allowing for customized training
    based on provided parameters.

    Parameters:
        nsub: Number of samples to subset from training data.
...
def save_results(folder, sroot):
    """
    Saves the average precision results of cell segmentation models after running denoising and segmentation.

    This method performs evaluation of segmentation models using test images with induced noise,
    computes average precision scores for various configurations, and saves the results as NumPy
    files.

    Args:
        folder: The directory path where the images are stored.
        sroot: The root path for the pretrained denoise models.

    Returns:
        None: This method does not return a value. It saves results to .npy files.
    """
